...
export const isograms: WordListQuestion = {
	...
	correctAnswer: (answer: string, wordList: string[]) => {
		const answers = answer.split(",").map((word) => word.trim());
		const isograms = wordList.filter(
			(word) => new Set(word).size === word.length,
		);
		return (
			answers.every((word) => isograms.includes(word)) &&
			isograms.every((word) => answers.includes(word))
		);
	},
	points: 5,
};
...
export const addTwo: TwoNumberQuestion = {
	...
	question: ([a, b]: [number, number]) =>
		`Calculate the sum (prefix polish notation) of: + ${a} ${b}`,
